...
export async function GET(req: NextRequest) {
  ...
  return new NextResponse(
    `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Vendor Approved</title>
    <style>body{font-family:sans-serif;max-width:500px;margin:80px auto;text-align:center;color:#111;}
    h1{font-size:28px;} p{color:#666;line-height:1.7;}</style></head>
    <body>
      <h1>✓ Approved</h1>
      <p><strong>${name}</strong> (${email}) has been approved.<br/>
      Their portal account is ready and they've received the welcome email.</p>
    </body></html>`,
